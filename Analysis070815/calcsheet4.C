...
#define NUMFILES 40
#define NUMOFFSETPTS 8 //number of temperature points in toffsets.txt
...
int main() {
  ...
  ofstream outfile;
  ifstream keyfile;
  ifstream tfile; //temperature file (toffsets.txt)
  std::string header;
  char entry[20];
  ...
  ifstream tfile;
  float cal_temp_apd16[NUMTPTS]; //calibrated temperature
  float cal_temp_apd17[NUMTPTS];
  float cal_temp_apd18[NUMTPTS];
  float cal_temp_apd19[NUMTPTS];
  int offsetpt;
  float temp_offsets[NUMOFFSETPTS]; //temperature corresponding to offsets
  ...
  tfile.open("toffsets.C");
  for (offsetpt=0;offsetpt<NUMOFFSETPTS;offsetpt++){
    tfile.getline(entry,20,'\t');
    stringstream(entry) >> temp_offsets[offsetpt];
    tfile.getline(entry,20,'\t');
    stringstream(entry) >> offset_apd16[offsetpt];
    tfile.getline(entry,20,'\t');
    stringstream(entry) >> offset_apd17[offsetpt];
    tfile.getline(entry,20,'\t');
    stringstream(entry) >> offset_apd18[offsetpt];
    tfile.getline(entry,20,'\n');
    stringstream(entry) >> offset_apd19[offsetpt];
  }
  

  //read and discard keyfile header
  ...
  for(filenum=0;filenum<NUMFILES;filenum++){
    ...
    //read in data from keyfile
    keyfile.getline(entry,20,'\t');
    stringstream(entry) >> apdnum;
    keyfile.getline(filename,40,'\t');
    keyfile.getline(entry,20,'\t');
    stringstream(entry) >> set_h2o_temp;
    keyfile.getline(entry,20,'\t');
    keyfile.getline(entry,20,'\t');
    stringstream(entry) >> set_tec_temp;
    keyfile.getline(entry,20,'\n');
    stringstream(entry) >> breakdownV;
    
    //cout << apdnum << "\t" << filename << "\t" 
    //  << set_h20_temp << "\t" << set_tec_temp << "\t" <<  breakdownV <<"\n";

    //open file with data to read
    ...
    for(numlines=0;numlines<LENGTH;numlines++){
      ...
      stringstream(entry) >> shutter_open[numlines];
      infile.getline(entry,20,'\t');
      stringstream(entry) >> degc[numlines];
      infile.getline(entry,20,'\t');
      stringstream(entry) >> h20in[numlines];
      infile.getline(entry,20,'\t');
      stringstream(entry) >> h2oout[numlines];
      infile.getline(entry,20,'\t');
      stringstream(entry) >> te_amps[numlines];
      infile.getline(entry,20,'\t');
      stringstream(entry) >> te_volts[numlines];
      infile.getline(entry,20,'\t');
      stringstream(entry) >> biasV[numlines];
      infile.getline(entry,20,'\t');
      stringstream(entry) >> ke237_amps[numlines];
      infile.getline(entry,20,'\t');
      stringstream(entry) >> pixel_sum[numlines];
      
      for(pixelnum=0;pixelnum<NUMPIXELS-1;pixelnum++){
	infile.getline(entry,20,'\t');
	stringstream(entry) >> pixels[numlines][pixelnum];
      }
      infile.getline(entry,20,'\n');
      stringstream(entry) >> pixels[numlines][NUMPIXELS-1];
    ...
    //cout << "calibrated pixels " << calibrated_pixels[LENGTH-1][0] <<"\n";
    ...
    //    cout << "photocurrent " << photo_current[0][0] << "\t"
    //	 <<photo_current[NUMVOLTAGEPTS-1][0] <<"\n";
	...
	running_total=running_total+calibrated_pixels[numlines][pixelnum];
      ...
      //cout << avg_calibrated_current[pixelnum] <<"\n";
    }
    
    //find avg_highbiasphoto_current
    for(pixelnum=0;pixelnum<NUMPIXELS;pixelnum++){
      running_total=0.;
      for(numlines=NUMVOLTAGEPTS-30;numlines<LENGTH;numlines++){
	running_total=running_total+photo_current[numlines][pixelnum];
      }
      //cout << photo_current[NUMVOLTAGEPTS-30][pixelnum] <<"\n";
      avg_highbiasphoto_current[pixelnum]=running_total/(1.0*NUMPIXELS*30.);
      //cout <<avg_highbiasphoto_current[pixelnum] << "\n"; 
    }
    ...
    int bad_data_run=0; //is the max voltage too low for the data run? 
    //if so, retake data later.

    if(top>=NUMVOLTAGEPTS){
      top=NUMVOLTAGEPTS-1;
      bottom=NUMVOLTAGEPTS-2;
      bad_data_run=1;
    }

    //find weight for interpolation. does this work if weight>1? I think so
    float weight;
    ...
    numpixelsinsum=0;
    for(pixelnum=0;pixelnum<NUMPIXELS;pixelnum++){
      if(bulk_dark_current[top][pixelnum]<999.){
	numpixelsinsum++;
	running_total=running_total+bulk_dark_current[top][pixelnum];
      }
    }
    avg_bulk_dc_top=running_total/(1.0*numpixelsinsum);
    for(pixelnum=0;pixelnum<NUMPIXELS;pixelnum++){
      if(bulk_dark_current[bottom][pixelnum]<999.){
	numpixelsinsum++;
	running_total=running_total+bulk_dark_current[bottom][pixelnum];
      }
    }
    avg_bulk_dc_bottom=running_total/(1.0*numpixelsinsum);
    ...
    for(pixelnum=0;pixelnum<NUMPIXELS;pixelnum++){
      if((invMdMdV[NUMVOLTAGEPTS-1][pixelnum]>-998.)&&
	 (invMdMdV[2][pixelnum]>-998.)){
	avg_slope_invMdMdV[pixelnum]=
	  (invMdMdV[NUMVOLTAGEPTS-1][pixelnum]-invMdMdV[2][pixelnum])/
	  (biasV[NUMVOLTAGEPTS-1]-biasV[2]);
      }
      else{
	avg_slope_invMdMdV[pixelnum]=-999.;
      }
    }
    running_total=0.;
    numpixelsinsum=0;
    for(pixelnum=0;pixelnum<NUMPIXELS;pixelnum++){
      if(avg_slope_invMdMdV[pixelnum]>-998.){
	numpixelsinsum++;
	running_total=running_total+avg_slope_invMdMdV[pixelnum];
      }
    }     
    apd_avg_slope_invMdMdV=running_total/(1.*numpixelsinsum);
